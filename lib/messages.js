...
function buildAgentPolicy(tools) {
...
export function buildAgentMessages(session, tools = []) {
  const messages = [];
  const base = appendRuntimeDateContext(
    session.systemPrompt || DEFAULT_SYSTEM_PROMPT,
  );
  const system = [base, ...buildAgentPolicy(tools)]
    .filter(Boolean)
    .join("\n\n");
  if (system) messages.push({ role: "system", content: system });
...
export function exportSessionTrace(
  session,
  { agentMode, grammarConfig = { grammarMode: "off" } } = {},
) {
  ...
  return {
    ...
    promptLayers: {
      ...
      agentPolicy: buildAgentPolicy(activeTools),
    },
