...
import { findRelevantSearchMemoryBlock } from "./search-memory.js";
...
export function buildMessages(session, grammarConfig, options = {}) {
  if (options.agentMode) {
    return buildAgentMessages(session, options);
  }
  const msgs = [];
...
/**
 * Messages for Gemma tool-calling agent turns (no grammar suffix).
 * @param {object} session
 * @param {{ ephemeral?: object[] }} [options]
 */
export function buildAgentMessages(session, options = {}) {
  const msgs = [];
  const base = (session.systemPrompt || "").trim();
  const memoryBlock = findRelevantSearchMemoryBlock(session, options.userMessage || "");
  const sys = [base, TOOL_USE_INSTRUCTION, TOOL_ANSWER_INSTRUCTION, TOOL_SYSTEM_GUARD, memoryBlock]
    .filter(Boolean)
    .join("\n\n");
  if (sys) msgs.push({ role: "system", content: sys });
  for (const m of session.messages) {
    if (m.role === "user") msgs.push({ role: "user", content: m.content });
      ...
      // Prior turns: final assistant prose only — do not replay tool protocol.
