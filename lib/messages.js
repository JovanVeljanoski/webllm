...
import { expandFileReferences } from "./file-context.js";
import { sanitizeExternalText } from "./sanitize.js";
import { splitThinking } from "./tool-parser.js";
import { splitBonsaiThinking } from "./bonsai-tool-parser.js";
export { splitThinking } from "./tool-parser.js";

export function splitModelThinking(raw, runtime) {
  if (runtime === "bonsai") return splitBonsaiThinking(raw);
...
export function applyAgentPolicy(
  ...
  const policy = buildAgentPolicy(tools, toolProtocol);
  if (!policy.length) return messages;
  ...
  if (systemIndex === -1) {
    output.unshift({ role: "system", content: policyText });
  } else {
...
export function buildAgentMessages(
  ...
  if (base) messages.push({ role: "system", content: base });
  messages.push(...expandFileReferences(session.messages || [], attachments, modelId));
  return applyAgentPolicy(messages, tools, { toolProtocol });
}
...
export function exportSessionTrace(
  ...
  const exactRequests = Array.isArray(runtimeTrace?.requests)
    ? runtimeTrace.requests
    : [];
  const exactLastRequest = exactRequests.at(-1) || null;
  const effectiveAgentMode =
    agentMode ?? (session.lastExecution?.mode === "agent");
  const activeTools = effectiveAgentMode ? tools : [];
  const reconstructedMessages = effectiveAgentMode
    ? buildAgentMessages(session, activeTools, {
      toolProtocol,
      attachments: referenceAttachments,
      modelId: session.modelId,
    })
    : buildMessages(session, grammarConfig);
  const modelContextMessages = exactLastRequest
    ? structuredClone(exactLastRequest.messages || [])
    : reconstructedMessages.map(toOpenAIMessage).filter(Boolean);
  const modelContextTools = exactLastRequest
    ? structuredClone(exactLastRequest.tools || [])
    : activeTools.map(tool => tool.schema);
  const attachmentSnapshot = exactLastRequest && Array.isArray(runtimeTrace?.attachments)
    ? runtimeTrace.attachments
    : attachments;
  return {
    format: "webllm-full-trace",
    version: 3,
    provenance: exactLastRequest ? "exact_runtime_capture" : "reconstructed_current_context",
    runtimeCapture: exactLastRequest
      ? {
        capturedAt: runtimeTrace.capturedAt || null,
        modelId: runtimeTrace.modelId || session.modelId || null,
      }
      : null,
    session: {
      id: session.id || null,
    ...
    runtimeRequests: exactRequests.map(request => structuredClone(request)),
    promptLayers: exactLastRequest
