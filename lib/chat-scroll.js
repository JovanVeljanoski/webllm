 ...
 * @param {{ force?: boolean, thresholdPx?: number, pinned?: boolean }} [opts]
 */
export function scrollToBottomIfPinned(
  el,
  { force = false, thresholdPx = CHAT_SCROLL_PIN_THRESHOLD_PX, pinned } = {},
) {
  if (!el) return;
  if (force || (pinned ?? isPinnedToBottom(el, thresholdPx))) {
