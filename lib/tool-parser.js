...
import { looksLikeBonsaiToolCallSyntax, stripBonsaiToolCallSyntax } from "./bonsai-tool-parser.js";
...
export function looksLikeToolCallSyntax(text) {
  ...
  if (looksLikeBonsaiToolCallSyntax(t)) return true;
...
export function stripToolCallSyntax(text, toolNames = ["web_search"]) {
  const src = stripBonsaiToolCallSyntax(stripLfmToolCallSyntax(text || ""), toolNames);
